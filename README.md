...
- **AI 自动回复**：HR 发来消息时，AI 结合你的简历和岗位信息回复，只在白天的回复时段工作；HR 主动找你的新岗位也会按求职条件判断并记录
- **消息记录**：和 HR 的聊天按会话集中查看，哪些是 AI 自动回复的一目了然
- **多种大模型可选**：默认使用 DeepSeek，也可以接入通义千问、Kimi、智谱、OpenAI 或本地 Ollama
- **岗位数据导出**：一键导出为 CSV，用 Excel 打开即可整理和复盘
...
- 查看岗位详情，或批量删除不需要的记录
- 点击「导出全部」把所有岗位导出为 CSV；勾选岗位后点击「导出」，只导出勾选的岗位。导出包含全部字段（岗位信息、HR、是否合适、判断描述、匹配度、是否已投递、入库时间等），Excel 打开中文不乱码

![岗位管理](assets/screenshots/jobs.png)

### 第六步：自动回复 HR 消息

1. 打开 **配置中心 → 求职配置 → 自动回复**，开启「启用自动回复」（需要先开通 AI 服务并上传简历）
2. 按需修改「回复提示词」，比如语气、要不要主动发简历、薪资怎么回答
3. 在最下方的「回复节奏」选择慢速 / 正常 / 快速，或自定义回复时段、每条回复前的等待时间和休息间隔；不在回复时段收到的消息，会顺延到下一个时段再回
4. 回到 **工作台**，点击「开始自动投递」下方的「开始自动回复」，可以和自动投递同时运行；需要结束时点击「停止回复」

HR 发来的消息和 AI 的回复都会显示在运行日志里。如果是 HR 主动找你、之前没看过的岗位，会先按求职条件和 AI 岗位筛选判断，再记录到岗位管理。

打开 **消息** 可以查看和每位 HR 的聊天记录：左侧是会话列表，支持按 HR、公司或岗位搜索；右侧是聊天内容和沟通的职位，AI 自动回复的消息会单独标注。

![消息](assets/screenshots/messages.png)
...
任何兼容 OpenAI 接口的服务都可以，例如通义千问、Kimi、智谱、OpenAI。在 **配置中心 → AI 服务** 的「接口地址」填写该服务的地址，再填 API Key、获取模型列表即可；用本地 Ollama 时填 `http://localhost:11434/v1`，API Key 随意填写。接口地址留空就是 DeepSeek。
